...
async def update_post(post_id: int, post_updates: PostUpdate, db: AsyncSession = Depends(get_db)):
...
async def delete_post(post_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if post_to_delete is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"The id: {post_id} you requested for does not exist")
